...
export function Checkbox({
  ...
  return (
    <fieldset className={`${checkbox.containerStyles ? checkbox.containerStyles : 'flex flex-col gap-y-nano'} group`}>
      <label className={`${checkbox.labelStyles ? checkbox.labelStyles : 'ds-small'} cursor-pointer group-has-disabled:cursor-not-allowed`} htmlFor={checkbox.id}>{checkbox.label}</label>
      <input id={checkbox.id} type="checkbox"
        ...
        className={`cursor-pointer w-[20px] h-[20px] relative appearance-none border-solid border-thin border-neutral-medium rounded-sm
          ...
          checked:after:scale-100 disabled:opacity-level-soft disabled:cursor-not-allowed ${checkbox.styles ?? ``}
        `}
      >
      </input>
    </fieldset>
